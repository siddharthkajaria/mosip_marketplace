...
class ProductImagesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create product_image" do
    assert_difference("ProductImage.count") do
      post product_images_url, params: { product_image: { image_data: @product_image.image_data, position: @product_image.position, product_id: @product_image.product_id, status: @product_image.status } }
    end
  ...
  test "should update product_image" do
    patch product_image_url(@product_image), params: { product_image: { image_data: @product_image.image_data, position: @product_image.position, product_id: @product_image.product_id, status: @product_image.status } }
